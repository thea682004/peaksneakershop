...
public class SupportSevice {
    ...
    public ResponseEntity<ResponseObject> saveOrUpdate(AddressRequest request) {
        ...
        if (addressOtp.isPresent()) {
            ...
            if (optCustomer.isEmpty()) {
                return new ResponseEntity<>(new ResponseObject("error", "Không tìm thấy customer", 0, request),
        ...
        if (request.getId() != null) {
            return new ResponseEntity<>(new ResponseObject("success", "Cập nhật thành công", 0, addressReturn),
                    HttpStatus.OK);
        } else {
            return new ResponseEntity<>(new ResponseObject("success", "Thêm mới thành công", 0, addressReturn),
                    HttpStatus.OK);
        }
    }
    ...
    public ResponseEntity<?> filterCustomers(Integer type) {
        ...
        if (type == 1) {
            ...
            lstCustomer = customerRepo.findAllCustomersWithInvoiceCreated(currentMonth,
                    TrangThaiBill.HOAN_THANH.getLabel());
        }
        List<CustomerResponse> resultDto = lstCustomer.stream()
    ...
    public ResponseEntity<?> findPromotionById(Long id) {
        ...
        if (otp.isEmpty()) {
            return new ResponseEntity<>(new ResponseObject("Fail", "Không tìm thấy id " + id, 1, null),
                    HttpStatus.BAD_REQUEST);
        }
        PromotionSupportResponse obj = otp.map(pro -> mapper.map(pro, PromotionSupportResponse.class))
    ...
    public ResponseEntity<?> findAllCustomerByDeleted(Boolean tyle) {
        ...
        List<CustomerResponse> resultDto = lstPromotion.stream()
                .map(promotion -> mapper.map(promotion, CustomerResponse.class)).collect(Collectors.toList());
        return new ResponseEntity<>(resultDto, HttpStatus.OK);
    }

    public ResponseEntity<?> findAllByDeleted(Boolean tyle) {
        List<Promotion> lstPromotion = promotionRepo.findAllByDeleted(tyle);
        List<PromotionSupportResponse> resultDto = lstPromotion.stream()
                .map(promotion -> mapper.map(promotion, PromotionSupportResponse.class)).collect(Collectors.toList());
        return new ResponseEntity<>(resultDto, HttpStatus.OK);
    }

    public ResponseEntity<?> findAllVoucherByDisabled() {
        List<Voucher> lstPromotion = voucherRepo.findAllByDeleted(false);
        List<VoucherResponse> resultDto = lstPromotion.stream()
                .map(promotion -> mapper.map(promotion, VoucherResponse.class)).collect(Collectors.toList());
        return new ResponseEntity<>(resultDto, HttpStatus.OK);
    }
    ...
    public ResponseEntity<?> saveOrUpdateUser(UserRequest request) {
        ...
        if (userOtp.isPresent()) {
            ...
            return new ResponseEntity<>(new ResponseObject("success", "Cập nhật thành công", 0, request),
                    HttpStatus.OK);
        } else {
            ...
            CompletableFuture<ResponseEntity<?>> saveTask = CompletableFuture.supplyAsync(() -> {
                ...
                return new ResponseEntity<>(new ResponseObject("success", "Thêm Mới Thành công", 0, request),
                        HttpStatus.OK);
            });
    ...
    public ResponseEntity<?> findAllByDeletedUsers(Boolean tyle) {
        ...
        List<UserResponse> resultDto = lstUser.stream().map(user -> mapper.map(user, UserResponse.class))
    ...
    public ResponseEntity<?> promotionUpadte(PromotionRequest request) {
        ...
        if (otpPromotion.isEmpty()) {
            return new ResponseEntity<>(new ResponseObject("Fail", "Không tìm thấy id " + request, 1, null),
                    HttpStatus.BAD_REQUEST);
        }
        ...
        if (lstProductDetails.size() == 0) {
            return new ResponseEntity<>(new ResponseObject("Fail", "Không có product detail" + request, 1, null),
                    HttpStatus.BAD_REQUEST);
        }
        ...
        for (Long id : productDetailsNotInPromotion) {
            ...
            if (otpProductDetails.isEmpty()) {
                return new ResponseEntity<>(new ResponseObject("Fail", "Không tìm thấy sản phẩm " + id, 1, null),
                        HttpStatus.BAD_REQUEST);
            }
            PromotionDetails promotionDetails = PromotionDetails.builder().promotion(promotion)
                    .productDetails(otpProductDetails.get()).build();
    ...
    public ResponseEntity<?> saveOrUpdateCustomer(CustomerSupportRequest customerDto, Long... idCustomer) {
        ...
        if (idCustomer.length <= 0) {
            if (customerRepo.existsByEmail(customerDto.getEmail())) {
                return new ResponseEntity<>(new ResponseObject("error",
                        "Email đã có trong hệ thống. Hãy sử dụng email khác", 1, customerDto), HttpStatus.BAD_REQUEST);
            }
            if (customerRepo.existsByPhone(customerDto.getPhone())) {
                return new ResponseEntity<>(
                        new ResponseObject("error",
                                "Số điện thoại đã có trong hệ thống. Hãy sử dụng số điện thoại khác", 1, customerDto),
                        HttpStatus.BAD_REQUEST);
            }
            // Lưu dữ liệu và thực hiện gửi email song song
            ...
            if (customerDto.getEmail() != null) {
                ...
                if (validateEmail.isPresent()) {
                    return new ResponseEntity<>(new ResponseObject("error", "Email đã tồn tại", 0, customerDto),
                            HttpStatus.BAD_REQUEST);
                }
            }

            if (validatePhone.isPresent()) {
                return new ResponseEntity<>(new ResponseObject("error", "Số điện thoại đã tồn tại", 0, customerDto),
                        HttpStatus.BAD_REQUEST);
            }
            ...
            CompletableFuture<ResponseEntity<?>> saveTask = CompletableFuture.supplyAsync(() -> {
                ...
                return new ResponseEntity<>(new ResponseObject("success", "Thêm Mới Thành công", 0, customerDto),
                        HttpStatus.OK);
            });
            ...
            if (customerOtp.isEmpty()) {
                return new ResponseEntity<>(
                        new ResponseObject("Fail", "Không tìm thấy khách hàng " + idCustomer[0], 1, null),
                        HttpStatus.BAD_REQUEST);
            }
            ...
            return new ResponseEntity<>(new ResponseObject("success", "Cập Nhật Thành công", 0, customerDto),
                    HttpStatus.OK);
    ...
    // paperFormat.setImageableArea(0, 0, paperFormat.getWidth(),
    ...
    //
    // // Each page you wish to print must be added to the `Book` as a printable
    // object.
    // // You can specify a width and height for `renderPage()` to get your desired
    // DPI when printing.
    ...
    //
    // // Here you can choose to display a print dialog before the actual print
    // call.
    ...
    public byte[] PrintInvoice(String billCode) throws Exception {
        ...
        props.put("receiverName",
                b.getBillFormat().equalsIgnoreCase(BillType.OFFLINE.getLabel())
                        ? b.getCustomer() != null ? b.getCustomer().getFullName() : null
                        : b.getReceiverName());
        props.put("receiverPhone",
                b.getBillFormat().equalsIgnoreCase(BillType.OFFLINE.getLabel())
    ...
    // public ResponseEntity<?> saveOrUpdateCustomer( CustomerSupportRequest
    // customerDto, Long ...idCustomer){
    // if (idCustomer.length <= 0){
    // Customer customer = new Customer();
    // customer = mapper.map(customerDto, Customer.class);
    // customer.setDeleted(false);
    ...
    // }).collect(Collectors.toList());
    // return new ResponseEntity<>(new ResponseObject("success","Thêm Mới Thành
    // công",0, customerDto), HttpStatus.OK);
    // }else {
    // Optional<Customer> customerOtp = customerRepo.findById(idCustomer[0]);
    // if (customerOtp.isEmpty()){
    // return new ResponseEntity<>(new ResponseObject("Fail", "Không tìm thấy khách
    // hàng " + idCustomer[0], 1, null), HttpStatus.BAD_REQUEST);
    // }
    // Customer customer = customerOtp.get();
    // Set<AddressRequest> lstAddressRequest = customerDto.getLstAddress();
    // Set<Address> lstAddress = customer.getLstAddress();
    ...
    // customer.setId(customerOtp.get().getId());
    // customerService.update(customer);
    // return new ResponseEntity<>(new ResponseObject("success","Cập Nhật Thành
    // công",0, customerDto), HttpStatus.OK);
    //
    // }
    //
