  ...
  const fillDataColumTable = (data) => {
    const dataTable = data?.sort((a, b) => new Date(a.createdDate) - new Date(b.createdDate)).map(data => {
      return {
        ...
        paymentMethod: <>
          <Tag color="#d98e3f"> {data.paymentMethod == "0" ? "Tiền mặt" : data.paymentMethod == "1" ? "Chuyển khoản" : "Phương thức khác"}</Tag>
        </>,
        paymentDate: dayjs(data?.paymentDate).format('HH:mm DD/MM/YYYY'),
        type: <>
          {data.type == "0" ? <Tag color="#d98e3f">Thanh toán</Tag> : <Tag color="#c71010">Hoàn tiền</Tag>}
  ...
  return (
    <>
      ...
      <div className='mt-2 mb-2 flex justify-end	mr-4'>
        Số tiền đã thanh toán: <span className="text-rose-600 font-medium ml-2">{fixMoney(amountPaid)}</span>
      </div>
    </>
  );
}
