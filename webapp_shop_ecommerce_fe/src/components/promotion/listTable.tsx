...
export default function ListTable() {
    ...
    const customStartDateFilter = (row, columnId, filterValue) => {
        if (filterValue == null) {
            return true;
        }
    ...
    const customEndDateFilter = (row, columnId, filterValue) => {
        if (filterValue == null) {
            return true;
        }
    ...
    const showDeleteConfirm = (id) => {
        confirm({
            ...
            onOk() {
                axios
                    .put(`${baseUrlV3}/promotion/disable/${id}`)
                    .then((res) => {
                                ...
                                <>
                                    {row.original.status == 0 && (
                                        <div
                                            ...
                                            onClick={() => {
                                                navigate(`/discount/promotion/update/${row.original.id}`);
                                            }}
                                        >
                                            <FaEdit />
                                            Cập nhật
                        ...
                        },
                        {
                            ...
                            label: (
                                <div
                                    className="flex gap-2 items-center"
                                    onClick={() => {
                                        navigate(`/discount/promotion/detail/${row.original.id}`);
                                    }}
                                >
                                    <FaEye />
                                    Chi tiết
                                ...
                                <>
                                    {row.original.status == 0 || row.original.status == 1 ? (
                                        <div
                                            className="flex gap-2 items-center"
                                            onClick={() => {
                                                showDeleteConfirm(row.original.id);
                                            }}
                                        >
                                            <FaTrash />
                                            Hủy
            ...
            <div className="bg-slate-50 rounded-md p-3 pb-6 shadow-lg" style={{ backgroundColor: '#fefce8', borderLeft: '4px solid #1e3a8a' }}>
                ...
                <div className="flex gap-5 items-center justify-between mt-4">
                    <Button
                        ...
                        variant="ountline"
                        onClick={() => {
                            navigate('/discount/promotion/add');
                        }}
                        className=""
                    >
...
const numberToPrice = (value) => {
    const formattedAmount = Number.parseFloat(value.toString()).toLocaleString('vi-VN', {
        style: 'currency',
        currency: 'VND',
    });
